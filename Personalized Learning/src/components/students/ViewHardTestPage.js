...
class ViewHardTestPage extends Component {
  state = {
    check: false,
    correctAnswers: 0,
    totalQuestions: 0,
  ...
  showResult = () => {
    ...
    if (grade < 0.119) {
      this.setState({
        resultMessage:
          "You have scored below the expected result of this test. Revisit the Easy learning material and then attempt the Easy test.",
      });
    } else if (grade < 0.269) {
      this.setState({
        resultMessage:
          "You have scored below the expected result of this test. Revisit the Medium learning material and then attempt the Medium test.",
      });
    } else if (grade >= 0.5) {
      this.setState({
        resultMessage:
          "Your result is good enough to be attempting the Hard level test. Keep up the good work.",
      });
    }
    this.setState({
      showResult: true,
  ...
  render() {
    ...
    if (module != undefined) {
      var test = Object.keys(module[0].tests.hard).map((key) => [
        key,
        module[0].tests.hard[key],
      ]);
      return (
        <div>
