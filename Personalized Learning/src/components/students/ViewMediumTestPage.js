...
class ViewMediumTestPage extends Component {
  state = {
    check: false,
    correctAnswers: 0,
    totalQuestions: 0,
  ...
  showResult = () => {
    ...
    if (grade < 0.5) {
      ...
      this.setState({
        resultMessage:
          "You have scored the expected result of this test, but not enough to advance to the next level. Revisit the Medium learning material and try to take this test again.",
      });
    } else if (grade >= 0.731) {
      this.setState({
        resultMessage:
  ...
  render() {
    ...
    if (module != undefined) {
      var test = Object.keys(module[0].tests.medium).map((key) => [
        key,
        module[0].tests.medium[key],
      ]);
      return (
        <div>
            ...
            <SubContainer>
              ...
              >
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <Timer
                    time={module[0].tests.medium.time}
